...
缓存互斥依赖设置页生命周期：HTTP 请求串行执行，音频接口要求设置页状态，退出设置会等待 `httpd_stop()` 完成，之后应用工作任务才能录音、绑定或回放。删除操作也在同一 HTTP 任务执行。若改为异步 HTTP，必须增加显式缓存占用管理。传输回调限定 15 秒，加上最多 5 秒 socket 发送超时；中途失败直接关闭分块响应，不发送成功结束标记。
